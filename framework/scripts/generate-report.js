...
function parseArgs(argv) {
  const args = {
    ...
    jsonSummaryFile:
      process.env.REGRESSION_JSON_SUMMARY ||
      "playwright-report/regression-summary.json",
    githubOutput: false,
  };
...
function main() {
  ...
  const summary = {
    ...
    runUrl: runUrl(),
    changedTests,
    executedTests: testCases.filter(
      (testCase) => testCase.status !== "skipped",
    ),
    passedTests,
    failedTests,
